...
class MLService:
    ...
    POSITIVE_WORDS = {
        ...
        "beautiful", "peaceful", "delighted", "excited", "thrilled", "grateful",
        "hopeful", "cheerful", "pleased", "satisfied", "content", "blessed"
    }
    
    NEGATIVE_WORDS = {
        "sad", "angry", "hate", "terrible", "awful", "horrible", "disgusting",
        "fearful", "anxious", "depressed", "miserable", "upset", "frustrated",
        "disappointed", "heartbroken", "devastated", "worried", "scared"
    }
    ...
    async def _extract_character_attributes(
        ...
        # Age pattern
        age_match = re.search(
            rf"{name}\s+(?:is|was)\s+(\d+)\s+years?\s+old",
            text, re.IGNORECASE
        )
        if age_match:
    ...
    async def parse_query(self, query: str) -> Dict[str, Any]:
        ...
        what_is_match = re.search(
            r"what\s+(?:is|are|was|were)\s+(\w+)(?:'s|s')?\s+(\w+)",
            query_lower
        )
        if what_is_match:
            return {
        ...
        if who_is_match:
            return {
                "type": "exists",
                "subject": who_is_match.group(1).title()
            }
        ...
        if exists_match:
            return {
                "type": "exists",
                "subject": exists_match.group(1).title()
            }
            
        # "Who is friends with X?" relationship pattern
        rel_match = re.search(
            r"who\s+(?:is|are|was|were)\s+(\w+)\s+(?:with|of|to)\s+(\w+)",
            query_lower
        )
        if rel_match:
            return {

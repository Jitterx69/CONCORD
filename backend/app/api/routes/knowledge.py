...
async def get_all_facts(req: Request, limit: int = 100, offset: int = 0):
    ...
    return {"facts": facts, "total": len(facts), "limit": limit, "offset": offset}
...
async def get_all_entities(
    req: Request, entity_type: Optional[EntityType] = None, limit: int = 100
):
    """
...
async def query_knowledge(query: str, req: Request):
    ...
    return {"query": query, "parsed": parsed, "results": results}
...
async def clear_knowledge_graph(req: Request, confirm: bool = False):
    ...
    if not confirm:
        raise HTTPException(
            status_code=400, detail="Must set confirm=true to clear knowledge graph"
        )

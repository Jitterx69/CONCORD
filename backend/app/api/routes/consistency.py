...
async def check_consistency(request: ConsistencyCheckRequest, req: Request):
    ...
    behavioral_score = 1.0 - (len([i for i in issues if i.type == ConstraintType.BEHAVIORAL]) * 0.15)
    relational_score = 1.0 - (len([i for i in issues if i.type == ConstraintType.RELATIONAL]) * 0.15)

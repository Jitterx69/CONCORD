            ...
            if fact.predicate == attribute:
                ...
                if not satisfied:
                    return ConsistencyIssue(
                        type=constraint.type,
                        level=ConsistencyLevel.INCONSISTENT if constraint.is_hard else ConsistencyLevel.WARNING,
    ...
    async def check_single(self, text: str, constraint_rule: str) -> Dict[str, Any]:
        ...
        if match:
            ...
            if operator == "==" and subject_in_text and expected_in_text:
                return {"satisfied": True, "explanation": f"Found {subject} with expected {attribute}"}

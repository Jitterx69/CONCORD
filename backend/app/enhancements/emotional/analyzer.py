...
class EmotionalAnalyzer:
    ...
    EMOTION_LEXICON = {
        "joy": {
            "words": ["happy", "joyful", "elated", "cheerful", "delighted", "ecstatic", "thrilled", "pleased", "content"],
            "intensity_modifiers": {"slightly": 0.3, "very": 0.8, "extremely": 1.0, "somewhat": 0.5}
        },
        "sadness": {
            "words": ["sad", "depressed", "melancholy", "sorrowful", "grief", "heartbroken", "miserable", "gloomy", "dejected"],
            "intensity_modifiers": {"slightly": 0.3, "deeply": 0.9, "profoundly": 1.0, "somewhat": 0.5}
        },
        "anger": {
            "words": ["angry", "furious", "enraged", "irritated", "annoyed", "outraged", "livid", "seething", "bitter"],
            "intensity_modifiers": {"slightly": 0.3, "extremely": 1.0, "mildly": 0.4}
        },
        "fear": {
            "words": ["afraid", "scared", "terrified", "anxious", "worried", "nervous", "panicked", "dreading", "frightened"],
            "intensity_modifiers": {"slightly": 0.3, "extremely": 1.0, "mortally": 1.0}
        },
        "love": {
            "words": ["love", "adore", "cherish", "affection", "devoted", "enamored", "passionate", "caring", "fond"],
            "intensity_modifiers": {"slightly": 0.3, "deeply": 0.9, "madly": 1.0}
        },
        "disgust": {
            "words": ["disgusted", "repulsed", "revolted", "sickened", "nauseated", "appalled", "horrified"],
            "intensity_modifiers": {"slightly": 0.3, "utterly": 1.0}
        },
        "surprise": {
            "words": ["surprised", "shocked", "amazed", "astonished", "stunned", "startled", "bewildered"],
    ...
    async def _detect_mood_shifts(
        ...
        for i in range(1, len(moods)):
            ...
            for pair in self.OPPOSING_EMOTIONS:
                if (prev_mood.mood in pair and curr_mood.mood in pair and 
                    prev_mood.mood != curr_mood.mood):
                    is_opposing = True
                    break
    ...
    async def check_tone_consistency(self, text: str) -> Dict[str, Any]:
        ...
        for i in range(1, len(tones)):
            ...
            elif (tones[i-1]["tone"] in ["formal", "informal"] and 
                  tones[i]["tone"] in ["formal", "informal"] and
                  tones[i-1]["tone"] != tones[i]["tone"]):
                inconsistencies.append({
                    "position": tones[i]["position"],
        ...
        return {
            ...
            "dominant_tone": max(set(t["tone"] for t in tones), 
                                key=lambda x: sum(1 for t in tones if t["tone"] == x))
            if tones else "neutral"
        }

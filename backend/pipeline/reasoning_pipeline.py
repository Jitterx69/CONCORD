    ...
    def retrieve_distributed(self, query, book_name=None, top_k=9):
        ...
        for name in books_to_search:
            ...
            for section_name, start, end in sections:
                ...
                sims = np.dot(section_emb, q) / (
                    np.linalg.norm(section_emb, axis=1) * np.linalg.norm(q) + 1e-9
                )
                    ...
                    if key in backstory_lower:
                        for opp in opposite_words:
                            if opp in evidence_lower:
                                violations.append(
                                    f"IMPLICIT: Backstory has '{key}' but evidence shows '{opp}'"
                                )
...
class TemporalConsistencyChecker:
    ...
    def check_character_arc(
        self, backstory: str, early_evidence: str, late_evidence: str, character: str
    ) -> Tuple[bool, str]:
...
class RuleBasedChecker:
    ...
    def check(content, char):
        ...
        action_after = re.search(
            r"(?:continued|after\s+(?:his|her)\s+death|until\s+)(\d{4})", content_lower
        )

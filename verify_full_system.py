...
def verify_system():
    ...
    # Expects Body
    res = requests.post(f"{BASE_URL}/api/v1/simulation/session/start", json={"mode": "test"})
    if check("Start Session", res):
        print(f"   Response: {res.json()}")
        
    # Expects Query Params
    res = requests.post(f"{BASE_URL}/api/v1/simulation/action", params={"character": "Alice", "action": "run"})
    check("Process Action", res)

    # 3. Causality (Butterfly Effect)
    print("\n--- Verifying Causal Propagator ---")
    # Check trace (GET)
    res = requests.get(f"{BASE_URL}/api/v1/causality/trace/00000000-0000-0000-0000-000000000000")
    ...
    res = requests.post(f"{BASE_URL}/api/v1/agents/check-consistency", params={"entity_id": dummy_uuid, "action": "run"})
